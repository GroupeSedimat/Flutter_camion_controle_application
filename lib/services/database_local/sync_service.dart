...
class SyncService {
  ...
  final DatabaseCamionTypeService firebaseCamionTypeService =
      DatabaseCamionTypeService();
  final DatabaseEquipmentService firebaseEquipmentService =
      DatabaseEquipmentService();
  final DatabaseCompanyService firebaseCompanyService =
      DatabaseCompanyService();
  final DatabaseListOfListsService firebaseLOLService =
      DatabaseListOfListsService();
  final DatabaseTasksService firebaseTaskService = DatabaseTasksService();
  final DatabaseBlueprintsService firebaseBlueprintService =
      DatabaseBlueprintsService();

  SyncService(this.db, this.networkService);

  Future<void> fullSyncTable(String tableName,
      {MyUser? user, String? userId, List<String>? dataPlus}) async {
      ...
      if (user != null) {
        if (dataPlus != null) {
          itsOk = await syncFromFirebase(tableName, timeSync,
              user: user, userId: userId, dataPlus: dataPlus);
        } else {
          itsOk = await syncFromFirebase(tableName, timeSync,
  ...
  Future<bool> syncFromFirebase(String tableName, String timeSync,
      {MyUser? user, String? userId, List<String>? dataPlus}) async {
    bool itsOk = true;
    String lastSync = "";
    ...
    switch (tableName) {
      case "users":
        ...
        await db.transaction((txn) async {
          ...
          if (user.role == "superadmin") {
            final Map<String, MyUser> firebaseUsers = await firebaseUserService
                .getAllUsersDataSinceLastSync(lastSync);
            final Map<String, MyUser>? localUsers =
                await getAllUsersSinceLastUpdate(txn, lastSync, timeSync);
            print(
                "----------- Firebase Users for superadmin since last update $firebaseUsers");
            ...
            for (var firebaseUser in firebaseUsers.entries) {
              ...
              if (localUser == null) {
                ...
                await insertUser(
                    txn, firebaseUser.value, firebaseUser.key, thisUser);
              } else {
                if (!firebaseUser.value.updatedAt
                    .isAtSameMomentAs(localUser.updatedAt)) {
                  conflicts.add({
            ...
            final Map<String, MyUser> firebaseUsers = await firebaseUserService
                .getCompanyUsersDataSinceLastSync(lastSync, companyName);
            final Map<String, MyUser>? localUsers =
                await getAllCompanyUsersSinceLastUpdate(
                    txn, lastSync, timeSync, companyName);
            print(
                "----------- Firebase Users for admin since last update $firebaseUsers");
            ...
            try {
              final Map<String, MyUser> firebaseUsers =
                  await firebaseUserService.getCurrentUserMapSinceLastSync(
                      lastSync, userId);
              final Map<String, MyUser>? localUsers =
                  await getUserDataSinceLastUpdate(
                      txn, lastSync, timeSync, userId);
              print(
                  "----------- Firebase Users for standard user since last update $firebaseUsers");
        ...
        await db.transaction((txn) async {
          ...
          if (user.role == "superadmin") {
            final Map<String, Camion> firebaseCamions =
                await firebaseCamionService
                    .getAllCamionsSinceLastSync(lastSync);
            final Map<String, Camion>? localCamions =
                await getAllCamionsSinceLastUpdate(txn, lastSync, timeSync);
            print(
                "----------- Firebase camions since last update $firebaseCamions");

            for (var firebaseCamion in firebaseCamions.entries) {
              final Camion? localCamion = localCamions?[firebaseCamion.key];
              if (localCamion == null) {
                await insertCamion(
                    txn, firebaseCamion.value, firebaseCamion.key);
              } else {
                if (!firebaseCamion.value.updatedAt
                    .isAtSameMomentAs(localCamion.updatedAt)) {
                  conflicts.add({
            ...
            final Map<String, Camion> firebaseCamions =
                await firebaseCamionService.getCompanyCamionsSinceLastSync(
                    user.company, lastSync);
            final Map<String, Camion>? localCamions =
                await getAllCamionsSinceLastUpdate(txn, lastSync, timeSync);
            print(
                "----------- Firebase camions since last update $firebaseCamions");
            ...
            final Map<String, Camion> firebaseCamions =
                await firebaseCamionService.getListCamionSinceLastSync(
                    user.camion!, lastSync);
            final Map<String, Camion>? localCamions =
                await getAllCamionsSinceLastUpdate(txn, lastSync, timeSync);
            print(
                "----------- Firebase camions since last update $firebaseCamions");
      ...
      case "camionTypes":
        ...
        await db.transaction((txn) async {
          ...
          if (dataPlus == null) {
            firebaseCamionTypes = await firebaseCamionTypeService
                .getAllCamionTypesSinceLastSync(lastSync);
          } else {
            firebaseCamionTypes = await firebaseCamionTypeService
                .getListedCamionTypesSinceLastSync(lastSync, dataPlus);
          }
          final Map<String, CamionType>? localCamionTypes =
              await getAllCamionTypesSinceLastUpdate(txn, lastSync, timeSync);
          print(
              "----------- Firebase camion types since last update $firebaseCamionTypes");

          for (var firebaseCamionType in firebaseCamionTypes.entries) {
            final CamionType? localCamionType =
                localCamionTypes?[firebaseCamionType.key];
            if (localCamionType == null) {
              await insertCamionType(
                  txn, firebaseCamionType.value, firebaseCamionType.key);
            } else {
              if (!firebaseCamionType.value.updatedAt
                  .isAtSameMomentAs(localCamionType.updatedAt)) {
                conflicts.add({
        ...
        await db.transaction((txn) async {
          if (userId == "123456789") {
            Map<String, String> companiesNames =
                await firebaseCompanyService.getAllCompaniesNames();
            print("Names ok");
            for (var firebaseCompany in companiesNames.entries) {
              insertCompanyName(
                  txn, firebaseCompany.value, firebaseCompany.key);
            }
          } else if (user == null) {
            print("user or userId not provided");
          } else {
            if (user.role == "superadmin") {
              ...
              final Map<String, Company> firebaseCompanies =
                  await firebaseCompanyService
                      .getAllCompaniesSinceLastSync(lastSync);
              final Map<String, Company>? localCompanies =
                  await getAllCompaniesSinceLastUpdate(txn, lastSync, timeSync);
              print(
                  "----------- Firebase Companies since last update $firebaseCompanies");

              for (var firebaseCompany in firebaseCompanies.entries) {
                final Company? localCompany =
                    localCompanies?[firebaseCompany.key];
                if (localCompany == null) {
                  await insertCompany(
                      txn, firebaseCompany.value, firebaseCompany.key);
                } else {
                  if (!firebaseCompany.value.updatedAt
                      .isAtSameMomentAs(localCompany.updatedAt)) {
                    conflicts.add({
              ...
              print("----------- sync service From Firebase Companies");
              final Map<String, Company> firebaseCompanies =
                  await firebaseCompanyService.getCompanyByIdSinceLastSync(
                      lastSync, user.company);
              final Map<String, Company>? localCompanies =
                  await getAllCompaniesSinceLastUpdate(txn, lastSync, timeSync);
              print(
                  "----------- Firebase Companies since last update $firebaseCompanies");
      ...
      case "listOfLists":
        ...
        await db.transaction((txn) async {
          ...
          if (dataPlus != null) {
            firebaseLoL =
                await firebaseLOLService.getLoLsWithIds(lastSync, dataPlus);
          } else {
            firebaseLoL =
                await firebaseLOLService.getAllLOLSinceLastSync(lastSync);
          }
          final Map<String, ListOfLists>? localLoL =
              await getAllListsSinceLastUpdate(txn, lastSync, timeSync);
          ...
          for (var firebaseList in firebaseLoL.entries) {
            ...
            if (localList == null) {
              ...
              if (!firebaseList.value.updatedAt
                  .isAtSameMomentAs(localList.updatedAt)) {
                conflicts.add({
      ...
      case "validateTasks":
        ...
        await db.transaction((txn) async {
          ...
          final Map<String, TaskChecklist> firebaseTasks =
              await firebaseTaskService.getAllTasks(userId);
          String lastUpdate =
              (await getOneWithName(txn, tableName))?.lastLocalUpdate ?? "";
      ...
      case "equipments":
        ...
        await db.transaction((txn) async {
          ...
          final Map<String, Equipment> firebaseEquipments =
              await firebaseEquipmentService
                  .getAllEquipmentsSinceLastSync(lastSync);
          final Map<String, Equipment>? localEquipmentsTypes =
              await getAllEquipmentsSinceLastUpdate(txn, lastSync, timeSync);
          print(
              "----------- Firebase Equipments since last update $firebaseEquipments");

          for (var firebaseEquipment in firebaseEquipments.entries) {
            final Equipment? localEquipment =
                localEquipmentsTypes?[firebaseEquipment.key];
            if (localEquipment == null) {
              await insertEquipment(
                  txn, firebaseEquipment.value, firebaseEquipment.key);
            } else {
              if (!firebaseEquipment.value.updatedAt
                  .isAtSameMomentAs(localEquipment.updatedAt)) {
                conflicts.add({
      ...
      case "blueprints":
        ...
        await db.transaction((txn) async {
          ...
          final Map<String, Blueprint> firebaseBlueprints =
              await firebaseBlueprintService
                  .getAllBlueprintsSinceLastSync(lastSync);
          final Map<String, Blueprint>? localBlueprintsTypes =
              await getAllBlueprintsSinceLastUpdate(txn, lastSync, timeSync);
          print(
              "----------- Firebase Blueprints since last update $firebaseBlueprints");

          for (var firebaseBlueprint in firebaseBlueprints.entries) {
            final Blueprint? localBlueprint =
                localBlueprintsTypes?[firebaseBlueprint.key];
            if (localBlueprint == null) {
              await insertBlueprint(
                  txn, firebaseBlueprint.value, firebaseBlueprint.key);
              for (String imageName in firebaseBlueprint.value.photoFilePath!) {
                Uint8List? imageData = await databaseImageService
                    .downloadBlueprintImageFromFirebase(imageName);
              ...
              if (!firebaseBlueprint.value.updatedAt
                  .isAtSameMomentAs(localBlueprint.updatedAt)) {
                conflicts.add({
      ...
      case "pdf":
        ...
        print(
            "There is nothing here, you don't have to download PDFs from the server right away");
        break;
      default:
        throw "Invalid Table";
    }
  ...
  Future<void> syncToFirebase(String tableName, String timeSync,
      {MyUser? user, String? userId}) async {
    ...
    switch (tableName) {
      case "users":
        ...
        final Map<String, MyUser>? localUsers =
            await getAllUsersSinceLastUpdate(db, lastUpdated, timeSync);
        print("----------- Users since last update $localUsers");
        if (localUsers != null) {
          for (var user in localUsers.entries) {
            print(
                "updating user ${user.key} ${user.value.role} ${user.value.username}");
      ...
      case "camions":
        ...
        final Map<String, Camion>? localCamions =
            await getAllCamionsSinceLastUpdate(db, lastUpdated, timeSync);
        ...
        if (localCamions != null) {
          for (var camion in localCamions.entries) {
            if (camion.key == "") {
              ...
              String newID =
                  await firebaseCamionService.addCamion(camion.value);
              await updateCamionFirebaseID(db, camion.value, newID);
            } else {
              await firebaseCamionService.updateCamion(
                  camion.key, camion.value);
            }
          }
        }
        print("-----------sync service To Firebase end");
      ...
      case "camionTypes":
        ...
        if (lastUpdated == "") {
          print(
              "-----------sync service To Firebase camion types sync no needed");
          break;
        }
        final Map<String, CamionType>? localCamionTypes =
            await getAllCamionTypesSinceLastUpdate(db, lastUpdated, timeSync);
        ...
        if (localCamionTypes != null) {
          for (var camionType in localCamionTypes.entries) {
            if (camionType.key == "") {
              ...
              String newID = await firebaseCamionTypeService
                  .addCamionType(camionType.value);
              await updateCamionTypesFirebaseID(db, camionType.value, newID);
            } else {
              await firebaseCamionTypeService.updateCamionType(
                  camionType.key, camionType.value);
            }
          }
        }
        print("-----------sync service To Firebase end");
      ...
      case "companies":
        ...
        final Map<String, Company>? localCompanies =
            await getAllCompaniesSinceLastUpdate(db, lastUpdated, timeSync);
        ...
        if (localCompanies != null) {
          for (var company in localCompanies.entries) {
            if (company.key == "") {
              ...
              String newID =
                  await firebaseCompanyService.addCompany(company.value);
              await updateCompanyFirebaseID(db, company.value, newID);
            } else {
              await firebaseCompanyService.updateCompany(
      ...
      case "listOfLists":
        ...
        final Map<String, ListOfLists>? localListOfLists =
            await getAllListsSinceLastUpdate(db, lastUpdated, timeSync);
      ...
      case "validateTasks":
        ...
        if (userId == null || tableInfo == null) {
          print(
              "---------- /////// +++++++++ no userId provided or no table info");
          break;
        }
        final Map<String, TaskChecklist>? tasks =
            await getAllTasksOfUser(db, userId);
      ...
      case "equipments":
        ...
        if (lastUpdated == "") {
          print(
              "-----------sync service To Firebase Equipments sync no needed");
          break;
        }
        final Map<String, Equipment>? localEquipments =
            await getAllEquipmentsSinceLastUpdate(db, lastUpdated, timeSync);
        ...
        if (localEquipments != null) {
          for (var equipment in localEquipments.entries) {
            if (equipment.key == "") {
              ...
              String newID =
                  await firebaseEquipmentService.addEquipment(equipment.value);
              await updateEquipmentFirebaseID(db, equipment.value, newID);
            } else {
              await firebaseEquipmentService.updateEquipment(
      ...
      case "blueprints":
        ...
        if (lastUpdated == "") {
          print(
              "-----------sync service To Firebase Blueprints sync no needed");
          break;
        }
        final Map<String, Blueprint>? blueprints =
            await getAllBlueprintsSinceLastUpdate(db, lastUpdated, timeSync);
        ...
        if (blueprints != null) {
          for (var blueprint in blueprints.entries) {
            if (blueprint.key == "") {
              ...
              String newID =
                  await firebaseBlueprintService.addBlueprint(blueprint.value);
              print("new blueprint id: $newID");
              await updateBlueprintFirebaseID(db, blueprint.value, newID);
            } else {
              await firebaseBlueprintService.updateBlueprint(
                  blueprint.key, blueprint.value);
            }
            if (blueprint.value.photoFilePath != null) {
              for (String path in blueprint.value.photoFilePath!) {
                Directory appDocDir = await getApplicationSupportDirectory();
                databaseImageService.addBlueprintImageToFirebase(
                    appDocDir.path, path);
              }
            }
          }
  ...
  Future<void> resolveConflict(
    ...
    switch (tableName) {
      ...
      case "camionTypes":
        ...
        if (shouldContinue == 'firebase') {
          await updateCamionType(
              dbOrTxn, firebaseData as CamionType, firebaseKey);
        } else if (shouldContinue == 'local') {
      ...
      case "equipments":
        ...
        if (shouldContinue == 'firebase') {
          await updateEquipment(
              dbOrTxn, firebaseData as Equipment, firebaseKey);
        } else if (shouldContinue == 'local') {
      ...
      case "blueprints":
        ...
        if (shouldContinue == 'firebase') {
          await updateBlueprint(
              dbOrTxn, firebaseData as Blueprint, firebaseKey);
        } else if (shouldContinue == 'local') {
