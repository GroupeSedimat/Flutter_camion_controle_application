...
Future<void> insertEquipment(dynamic dbOrTxn, Equipment equipment, String firebaseId) async {
  try{
...
Future<void> updateEquipment(dynamic dbOrTxn, Equipment equipment, String firebaseId) async {
  try{
    ...
    print("Error while updating data for ${equipment.name} in table Equipments: $e");
  }
}

Future<void> updateEquipmentFirebaseID(Database db, Equipment equipment, String firebaseId) async {
...
Future<void> softDeleteEquipment(Database db, String firebaseId) async {
  try{
    ...
    print("Error while deleting data with id $firebaseId in table Equipments: $e");
  }
}
...
Future<void> restoreEquipment(Database db, String firebaseId) async {
  try{
    ...
    print("Error while restoring data with id $firebaseId in table Equipments: $e");
  }
}
...
Future<int> getFirstFreeEquipmentNumber(Database db) async {
  int lastEquipmentID = 0;
  try{
    final List<Map<String, dynamic>> maps = await db.query(tableName);
    if(maps.isEmpty){
      return 0;
    ...
    for (var equipmentItem in maps) {
      String id = equipmentItem["id"];
      if(id.length<10 && int.parse(id)>lastEquipmentID){
        lastEquipmentID = int.parse(id);
      }
    }
...
Future<Map<String,Equipment>?> getAllEquipmentsSinceLastUpdate(dynamic dbOrTxn, String lastUpdated, String timeSync) async {
  Map<String, Equipment> equipments = {};
  try {
    ...
    print("Error while getting all data from table Equipments since last actualisation: $e");
  }
  return sortedEquipments(equipments: equipments);
}

Future<void> insertMultipleEquipments(dynamic dbOrTxn, Map<String, Equipment> equipments) async {
  try {
...
Future<Equipment?> getOneEquipmentWithID(dynamic dbOrTxn, String equipmentID) async {
  try{
    final List<Map<String, dynamic>> maps = await dbOrTxn.query(
      tableName,
      where: 'id = ?',
    ...
    print("Error while getting data of Equipment with id $equipmentID from table Equipments: $e");
    return null;
  }
}
